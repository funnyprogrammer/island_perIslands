...
def runRound(par, parComb, parCount, logIndex, numberOfThreads, round, broadcast, messenger, islandSizes, percentageOfBestIndividualsForMigrationAllIslands, islandNumber):
    ...
    for currentGeneration in range(1, numberOfGenerations):
        ...
        if (fitnessEvolution[currentGeneration][10] >= simplicityStart) and (precisenessWeight > 0):
            simplicityWeight = float(par[islandNumber + parCount][21])
        print('LOG:', logIndex, '| COMB:', parComb, '| RND:', round, '| GEN:', currentGeneration, '| TF:', '%.6f' % highestValueAndPosition[0][0], '| C:', '%.6f' % highestValueAndPosition[0][1], '| TP:', '%.6f' % highestValueAndPosition[0][2], '| P:', '%.6f' % highestValueAndPosition[0][3], '| S:', '%.6f' % highestValueAndPosition[0][4], '| REP:', fitnessEvolution[currentGeneration][8], fitnessEvolution[currentGeneration][3], fitnessEvolution[currentGeneration][9], fitnessEvolution[currentGeneration][10], fitnessEvolution[currentGeneration][11], '| ISL:', islandNumber, '| PAR:', islandNumber)
    ...
    for logID in range(len(log_list)):
        ...
        for parComb in range(numberOfParametersCombinations):
            ...
            for round in range(numberOfRounds):
                ...
                broadcast.append(1)
                func2 = partial(func, round, broadcast, messenger, islandSizes, percentageOfBestIndividualsForMigrationAllIslands)
